...
public class Parser 
{
	...
	public static DoMathExpression parseDoMath(String expression)
	{
	...
	static WhileStatement parseWhile(TestExpression testExpression, Statement trueStatement)
	{
		WhileStatement qs = new WhileStatement(testExpression, trueStatement);
		return qs;
	}
	...
	static Statement parseStatement(String s)
	{
		...
		else if(theParts[0].equals("while"))
		{
			...
			String testExpression = expression.substring(0, posOfDoKeyword);
			expression = expression.substring(posOfDoKeyword + "do".length()).trim();
			//int posOfDomathKeyword = expression.indexOf("do-math");
			//String doStatement = expression.substring(0, posOfDomathKeyword).trim();
			//String doMathStatement = expression.substring(posOfDomathKeyword + "do-math".length()).trim();
			
			//return Parser.parseWhile(
			//				(TestExpression)Parser.parseExpression(testExpression), 
			//				Parser.parseStatement(doStatement), 
			//				Parser.parseStatement(doMathStatement));
			return Parser.parseWhile(
									(TestExpression)Parser.parseExpression(testExpression), 
								Parser.parseStatement(expression));
